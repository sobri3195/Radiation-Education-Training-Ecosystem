...
import { History, Search, Plus, Download, Edit, Trash2 } from 'lucide-react';
import DataTableWithExport from '@/components/DataTableWithExport';

export default function Simudose_HistoryPage() {
  const [searchTerm, setSearchTerm] = useState('');

  const columns = [
    { key: 'tanggal', label: 'Tanggal' }, { key: 'user', label: 'User' }, { key: 'jenis_kanker', label: 'Jenis Kanker' }, { key: 'dosis', label: 'Dosis' }, { key: 'hasil', label: 'Hasil' }
  ];
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Riwayat Perhitungan SimuDose</h1>
          <p className="text-slate-600 mt-1">Lihat riwayat perhitungan BED/EQD₂ yang telah dilakukan</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <DataTableWithExport
          ...
          exportFileName="simudose-history"
        />
      </div>
    </div>
  );
