...
import { AlertCircle, Search, Plus, Download, Edit, Trash2 } from 'lucide-react';
import DataTableWithExport from '@/components/DataTableWithExport';

export default function Oncomap_Side_EffectsPage() {
  const [searchTerm, setSearchTerm] = useState('');

  const columns = [
    { key: 'organ', label: 'Organ' }, { key: 'efek_samping', label: 'Efek Samping' }, { key: 'grade', label: 'Grade' }, { key: 'dosis_threshold', label: 'Dosis Threshold' }
  ];
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Efek Samping Terkait OAR</h1>
          <p className="text-slate-600 mt-1">Kelola data efek samping terkait organ at risk</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <DataTableWithExport
          ...
          exportFileName="oncomap-side-effects"
        />
      </div>
    </div>
  );
