...
import { Heart, Search, Plus, Download, Edit, Trash2 } from 'lucide-react';
import DataTableWithExport from '@/components/DataTableWithExport';

export default function Side_Effects_CarePage() {
  const [searchTerm, setSearchTerm] = useState('');

  const columns = [
    { key: 'efek_samping', label: 'Efek Samping' }, { key: 'anjuran', label: 'Anjuran' }, { key: 'kontraindikasi', label: 'Kontraindikasi' }
  ];
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Anjuran Perawatan</h1>
          <p className="text-slate-600 mt-1">Panduan perawatan mandiri untuk efek samping</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <DataTableWithExport
          ...
          exportFileName="side-effects-care"
        />
      </div>
    </div>
  );
