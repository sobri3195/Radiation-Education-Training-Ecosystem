...
import { Package, Search, Plus, Download, Edit, Trash2 } from 'lucide-react';
import DataTableWithExport from '@/components/DataTableWithExport';

export default function Minilab_PackagesPage() {
  const [searchTerm, setSearchTerm] = useState('');

  const columns = [
    { key: 'nama_paket', label: 'Nama Paket' }, { key: 'jumlah_soal', label: 'Jumlah Soal' }, { key: 'durasi', label: 'Durasi' }, { key: 'status', label: 'Status' }
  ];
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Paket Ujian</h1>
          <p className="text-slate-600 mt-1">Kelola paket soal dan ujian Mini-Lab</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <DataTableWithExport
          ...
          data={sampleData}
          title="Paket Ujian"
          exportFileName="minilab-packages"
        />
      </div>
    </div>
  );
