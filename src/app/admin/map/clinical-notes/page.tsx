...
import { Lightbulb, Search, Plus, Download, Edit, Trash2 } from 'lucide-react';
import DataTableWithExport from '@/components/DataTableWithExport';

export default function Map_Clinical_NotesPage() {
  const [searchTerm, setSearchTerm] = useState('');

  const columns = [
    { key: 'topik', label: 'Topik' }, { key: 'kategori', label: 'Kategori' }, { key: 'catatan', label: 'Catatan' }, { key: 'penulis', label: 'Penulis' }
  ];
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Catatan Klinis & Tips</h1>
          <p className="text-slate-600 mt-1">Catatan praktis dan tips klinis untuk praktisi</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <DataTableWithExport
          ...
          exportFileName="map-clinical-notes"
        />
      </div>
    </div>
  );
