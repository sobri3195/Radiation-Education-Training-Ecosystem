...
import { ClipboardList, Search, Plus, Download, Edit, Trash2 } from 'lucide-react';
import DataTableWithExport from '@/components/DataTableWithExport';

export default function Journey_Follow_UpPage() {
  const [searchTerm, setSearchTerm] = useState('');

  const columns = [
    { key: 'judul', label: 'Judul' }, { key: 'timeline', label: 'Timeline' }, { key: 'target', label: 'Target' }, { key: 'status', label: 'Status' }
  ];
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Edukasi Follow-up</h1>
          <p className="text-slate-600 mt-1">Materi edukasi untuk follow-up pasien</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <DataTableWithExport
          ...
          exportFileName="journey-follow-up"
        />
      </div>
    </div>
  );
